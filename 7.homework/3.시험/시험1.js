...
list.addEventListener("click", function (ev) {
  ...
  if (node.nodeName == "BUTTON") {
    // 버튼일때만 실행
    for (let i = 0; i < books.length; i++) {
      let click_price = parseInt(
        node.closest("tr").querySelector(".price").innerText
      ); // 내가 클릭한 행의 가격
      if (books[i].price == click_price) {
        // books배열의 가격중에 내가 클릭한 행의 가격이 같은것 찾기
